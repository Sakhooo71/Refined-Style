    ...
    public function new(Request $request, EntityManagerInterface $entityManager): Response
    {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            if ($file) {
                ...
                try {
                    $file->move(
                        $this->getParameter('uploads_categories_directory'),
                        $newFilename
                    );
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            if ($file) {
                ...
                try {
                    ...
                    if ($originalImage && file_exists($this->getParameter('uploads_categories_directory').'/'.$originalImage)) {
                        unlink($this->getParameter('uploads_categories_directory').'/'.$originalImage);
                    }
                    $category->setCategoryImage($newFilename);
                } catch (FileException $e) {
                    $this->logger->error(sprintf('An error occurred while uploading the file: %s', $e->getMessage()));
