...
class DefaultController extends AbstractController
{
    ...
    public function index(ProductRepository $productRepository, CategoryRepository $categoryRepository, CacheInterface $cache): Response
    {
        $products = $cache->get('latest_products', function (ItemInterface $item) use ($productRepository) {
            $item->expiresAfter(3600); // Cache for 1 hour
            return $productRepository->findBy([], ['id' => 'DESC'], 8);
        });

        $categories = $cache->get('all_categories', function (ItemInterface $item) use ($categoryRepository) {
            $item->expiresAfter(3600); // Cache for 1 hour
    ...
    public function categoryProducts($id, CategoryRepository $categoryRepository, ProductRepository $productRepository): Response
    {
        ...
        $products = $productRepository->findForCategoryPage($category);
